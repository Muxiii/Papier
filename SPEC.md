...
单页 Web 应用：**宽屏（视口宽度 > 960px）** 下左侧为 **约 280px 内联导航栏**（品牌、日期/待办切换、用户区），右侧为**双页并排日记画布**（相邻两个日历日；**每个日期固定落在左页或右页之一**，不随查看日左右对调）。**≤960px** 时侧栏改为**抽屉**，由左上角悬浮胶囊（Papier 图标 + 汉堡）打开；**≤680px** 时主日记区改为**单页**（仅当前查看日一页纸，空白处左半/右半点击分别向前/向后翻日）。用户可通过**底部 AI 对话**从自然语言生成贴纸备选并贴到画布；贴纸可**拖拽**、**缩放与旋转**、**双击查看详情**、**编辑（限今天及未来）**、**切换状态**、**删除**。
...
**断点**（`src/hooks/useMediaQuery.ts` + `App.tsx`）：`(max-width: 960px)` 启用抽屉侧栏；`(max-width: 680px)` 启用单页日记（见 2.2）。
...
- **`DiarySpread` 的 `layout`**：`spread`（默认）或 `single`（`App` 在 `≤680px` 时传入 `layout="single"`）。
...
- **单页（`single`）**：仅展示当前 `viewingDate` 对应的一页纸（`max-w-[640px]`），贴纸列表为当日 `viewingDate` 过滤结果；**无中间书脊、无双页右页蒙层**。点击该页空白区域：**点击位置在页宽左半** → 向前翻 1 天，**右半** → 向后翻 1 天（`onBlankClickSplit`）。其余贴纸交互与双页一致。
...
└── src/
    ├── main.tsx
    ├── index.css
    ├── App.tsx                 # hydration 门闸、侧栏抽屉/断点、导航 z-index、路由级 modal key
    ├── hooks/
    │   └── useMediaQuery.ts    # matchMedia 订阅，用于 960/680 断点
    ├── types/
    │   ├── sticker.ts
    ...
    └── components/
        ...
        ├── DiarySpread.tsx       # spread 双页 | single 单页；固定奇偶槽位与单页翻日空白分半
        ├── DateHeader.tsx
        ├── MiniCalendar.tsx
        ├── StickerCanvas.tsx
        ├── StickerCard.tsx
...
| 2026-04-21 | **响应式导航与日记**：`useMediaQuery`；`≤960px` 侧栏改为抽屉，左上角 Papier + 汉堡悬浮入口（抽屉打开时隐藏），蒙层/抽屉 z-index 高于 AI 与贴纸、低于贴纸详情弹窗；`≤680px` 日记为 `DiarySpread` 单页布局，空白左/右半翻日；`LeftSidebar` 支持 `inline` / `drawer`。 |
