...
单页 Web 应用：左侧为**内联导航栏**（约 **128px～280px**）；右侧为**双页并排日记**（半页设计宽约 **500px**，双页总宽约 **1024px** 含书脊）。视口变窄时顺序为：**先收主区日记左右白边** → **再收侧栏宽度** → **双页改为单页后再收白边** → 再窄则**抽屉 + 悬浮菜单**。主区与日记顶区铺 **`#e5e2dc`**，避免顶下移后出现露灰。底部 **AI 条**与日记纸共用 `padX` 且 `maxWidth` 与 `diaryContentWidthPx` 对齐。接近抽屉阈值前主区**顶向下渐移**。用户可通过**底部 AI 对话**生成贴纸备选并贴到画布；贴纸可**拖拽**、**缩放与旋转**、**双击查看详情**、**编辑（限今天及未来）**、**切换状态**、**删除**。
...
**视口布局**（`src/lib/diaryViewportLayout.ts` + `useDiaryViewportLayout` + `App.tsx`）：按 `window.innerWidth` 计算 `sidebarWidth`、`spreadMode`、`spreadScale`（当前恒为 1）、`padX`、`diaryContentWidthPx`、`useDrawer`、`diaryOffsetTop`（抽屉时额外下移）；无固定 `960/680` 媒体查询断点。
...
#### 抽屉模式（视口不足以容纳：最窄侧栏 + 单页纸宽 + 最小白边）

- `LeftSidebar` 支持 `mode="inline"`（宽屏）与 `mode="drawer"`（窄屏）；抽屉从左侧滑出，全屏暗色蒙层，点击蒙层或侧栏内关闭、**Esc** 关闭。
- **入口**：`fixed` 左上角悬浮胶囊（圆角全边框、浅底 + 阴影）：左侧 **`/papier-icon.png` 小图**（相对胶囊左缘使用较大 `padding-left` 留白），右侧 **汉堡图标**；`aria-label` 为「打开导航菜单」。**抽屉打开时该悬浮按钮不渲染**，避免与侧栏叠压。
- **层级**（`App.tsx` 常量，均高于 AI 抽屉约 `20000` 段与贴纸堆叠约 `10000`，低于贴纸详情弹窗 `StickerModal` 的 `z-[21000]`）：蒙层 `Z_NAV_MENU_BACKDROP = 20600`，抽屉面板 `Z_NAV_MENU_DRAWER = 20650`，关闭态入口 `Z_NAV_MENU_FAB = 20700`，保证展开菜单时不被贴纸与 AI 区域挡住。
- 从待办/日期列表切换日期时仍会关闭抽屉（与宽屏一致的导航行为）。

### 2.2 日记视图（双页 / 单页）

- **`DiarySpread` 的 `layout`**：`spread` 或 `single`；`padXPx` 主区水平白边；`spreadScale` 双页缩放（当前恒 1）；`singlePageWidthPx` 与设计半页宽（**500**）一致。
- **双页（`spread`）**：主区为书本摊开式双页：由 `spreadDatesForViewing(viewingDate)` 得到 `leftDate` / `rightDate`（相邻自然日）。**每个日历日按与固定锚点的日历日差奇偶性**（`datePageSlot`）永远落在左页或右页之一；切换查看日时，某一具体日期不会从左侧「挪到」右侧。
...
- **单页（`single`）**：仅展示当前 `viewingDate` 一页，宽 **500px**（`singlePageWidthPx`）；贴纸为当日过滤；**无书脊、无双页右页蒙层**。点击纸面空白：**左 1/3** 向前翻日，**右 1/3** 向后翻日，中间 1/3 不翻页（`onBlankClickSplit`）。`padXPx` 可收至 `PAD_MIN` 而纸宽不变。
...
- 贴纸 **done**：实色卡片；**todo**：半透明 + 虚线边框；**cancelled**：灰色虚线卡片（保留在原日期，可恢复待办）；**note**（Fragments）：近白色底卡 + 极浅横纹（笔记纸感），标题与正文为固定字号（标题小于正文）且不随卡片缩放变化；无额外「Fragments」副标签。
...
- **缩放点与旋转（类 Figma）**：四角 **18×18px** 缩放点热区（可见为约 **8×8px** 的小方块，直角无圆角，边缘色 `#D3BA92`，内部白色填充）；缩放点边缘线与选中描边统一为 **1.5px**，且缩放点图层高于选中描边（避免被挡住）。指针为斜向缩放；角外侧 **约 28px** 延伸的感应区主要在贴纸外，拖曳为**旋转**；旋转感应区使用四方向 SVG 旋转 cursor（`public/rotate-cursor-*.svg`）。旋转与拖拽移动都会触发 `滑动.MP3`。缩放点叠在感应区之上（z-index 更高）时优先缩放。无单独「旋转按钮」。
...
- **已完成**：「标记为未完成」。
- **待办（未完成）**：底部横向并列 **「已完成」**（底色 `#DEBD8C`、白字，与 AI 发送按钮一致）与 **「取消待办」**（状态改为 `cancelled` 并关闭弹窗）；下方为「删除贴纸」。
- **已取消（cancelled）**：在详情弹窗中显示「恢复待办」按钮，可恢复为 `todo`。
...
- 用户消息为右侧气泡：底色 `#F3ECE2`、更大内边距、圆角约 `15px`（右下角 `7px`）；AI 消息不使用气泡容器（纯文本块）。
- 消息条目垂直间距约 `28px`（`space-y-7`）。
...
└── src/
    ├── main.tsx
    ├── index.css
    ├── App.tsx                 # hydration、视口布局、抽屉/导航 z-index、modal key
    ├── hooks/
    │   ├── useDiaryViewportLayout.ts  # resize → diaryViewportLayout
    │   └── useMediaQuery.ts           # 保留；当前主布局不依赖
    ├── types/
    │   ├── sticker.ts
    ...
    │   ├── diaryViewportLayout.ts # 连续侧栏宽度、双页 scale、单页/抽屉阈值
    │   └── api.ts              # postChat(messages, context)
    └── components/
        ├── LeftSidebar.tsx       # 左侧导航：inline / drawer；日期/待办 Tab、总结、用户区
        ├── DiarySpread.tsx       # spread scale + single 固定纸宽；奇偶槽位；单页空白分半
        ├── DateHeader.tsx
        ├── MiniCalendar.tsx
        ├── StickerCanvas.tsx
        ├── StickerCard.tsx
...
| 2026-04-21 | **响应式导航与连续视口**：`diaryViewportLayout` + `useDiaryViewportLayout`（替代固定 960/680）；内联侧栏 128～280px 连续变窄 → 双页 `scale` 至 0.8 → 单页（纸宽=半页@0.8）→ 仅收主区 `padX` → 抽屉 + Papier/汉堡悬浮球（打开时隐藏）；z-index 与左/右半翻日行为同前；接近抽屉阈值时主区 `paddingTop` 渐增以免与悬浮球重叠。 |
| 2026-04-21 | **取消态待办与对话视觉**：贴纸新增 `cancelled` 状态；取消待办后以灰色贴纸保留在原日期，可在弹窗恢复为待办；取消项进入侧栏待办列表并置于最低优先级。AI 对话区改为用户气泡 `#F3ECE2`、AI 纯文本、消息间距 28px。 |
| 2026-04-21 | **旋转交互细节**：旋转贴纸时也触发 `滑动.MP3`；四角旋转感应区使用不同方向的 SVG 旋转 cursor（`public/rotate-cursor-nw/ne/se/sw.svg`）。 |
